...
def _safe_max_timestamp(ratings_df: pd.DataFrame) -> str:
...
def _artifact_version(movies_df: pd.DataFrame, ratings_df: pd.DataFrame, reviews_all: pd.DataFrame) -> str:
    max_timestamp = _safe_max_timestamp(ratings_df)
    raw = f"{len(movies_df)}|{len(ratings_df)}|{len(reviews_all)}|{max_timestamp}"
...
def main(output_dir: Path):
    ...
    if movies_df.empty:
        raise ValueError(
            "Cannot build recommendation artifacts: movies table is empty. Upload or insert movie metadata first."
        )

    if ratings_df.empty:
        raise ValueError(
            "Cannot build collaborative filtering artifacts: ratings table is empty. Upload or insert ratings first."
        )
